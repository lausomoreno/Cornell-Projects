...
float **block(float **x, int seqLength, int embeddingSize, BlockWeights weights) {
    ...
    for (int i = 0; i < seqLength; i++) {
        ...
        // Hint: it should be first_block_MLP followed by gelu, and then second_block_MLP
         ...
         // First linear layer: [embeddingSize] -> [4 * embeddingSize]
        float *hidden = linear(normalized_x_added[i],first_block_MLP.weights,first_block_MLP.biases,
        ...
        // Second linear layer: [4 * embeddingSize] -> [embeddingSize]
        float *out = linear(hidden_gelu,second_block_MLP.weights,second_block_MLP.biases,
