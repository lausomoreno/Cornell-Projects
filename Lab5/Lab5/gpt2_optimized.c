...
// Function prototypes
void linear(const float *fcInput,float **weights,const float *biases,int fcInputSize,int fcOutputSize,float *output);
...
void linear(const float * restrict fcInput,
            ...
            float * restrict output)
{
    for (int i = 0; i < fcOutputSize; ++i) {
        float acc = biases[i];
        float * restrict w_row = weights[i];

        int j = 0;
        // simple unrolling by 4 to help -O3 vectorize
        for (; j <= fcInputSize - 4; j += 4) {
            acc += fcInput[j]     * w_row[j]
                 + fcInput[j + 1] * w_row[j + 1]
                 + fcInput[j + 2] * w_row[j + 2]
                 + fcInput[j + 3] * w_row[j + 3];
        }
        for (; j < fcInputSize; ++j) {
            acc += fcInput[j] * w_row[j];
        }

        output[i] = acc;   // writes are sequential in i
    }
}
...
float **block(float **x, int seqLength, int embeddingSize, BlockWeights weights) {
    ...
    for (int i = 0; i < seqLength; i++) {
    Q[i] = (float *)malloc(q_mlp.fcOutputSize * sizeof(float));
    linear(normalized_x[i], q_mlp.weights, q_mlp.biases,
           q_mlp.fcInputSize, q_mlp.fcOutputSize, Q[i]);

    K[i] = (float *)malloc(k_mlp.fcOutputSize * sizeof(float));
    linear(normalized_x[i], k_mlp.weights, k_mlp.biases,
           k_mlp.fcInputSize, k_mlp.fcOutputSize, K[i]);

    V[i] = (float *)malloc(v_mlp.fcOutputSize * sizeof(float));
    linear(normalized_x[i], v_mlp.weights, v_mlp.biases,
           v_mlp.fcInputSize, v_mlp.fcOutputSize, V[i]);
    }
    ...
    for (int i = 0; i < seqLength; i++) {
        ...
        float *hidden = (float *)malloc(first_block_MLP.fcOutputSize * sizeof(float));
        linear(normalized_x_added[i],
       first_block_MLP.weights, first_block_MLP.biases,
       first_block_MLP.fcInputSize, first_block_MLP.fcOutputSize,
       hidden);

        // GELU activation
        float *hidden_gelu = gelu(hidden, first_block_MLP.fcOutputSize);
        free(hidden);

        // Second linear layer
        float *out = (float *)malloc(second_block_MLP.fcOutputSize * sizeof(float));
        linear(hidden_gelu,
       second_block_MLP.weights, second_block_MLP.biases,
...
float *model(int *tokens, int seqLength, GPT2Weights weights) {
    ...
    float *logits = (float *)malloc(logits_mlp.fcOutputSize * sizeof(float));
    linear(h[seqLength - 1],
       logits_mlp.weights, logits_mlp.biases,
